...
import {getToDoList, getDoingList} from '@/api/getData'
...
export default {
    ...
    methods: {
        async initData() {
            try {
                const queryResult = await getDoingList({"nowProcessorId": +sessionStorage.getItem('userId')});
                ...
                todoListData.forEach(item => {
                    ...
                    tableData.id = item.id;
                     tableData.carOwnerName = item.carOwnerName;
                     tableData.carOwnerPhone = item.carOwnerPhone;
                     tableData.marketingFee = item.marketingFee;
